...
# Phase 1.5:
# Keep optional semantic dependencies out of module import time.
# This allows the core application to import semantic_retrieval
# without constructing/downloading an embedding model.
...
def _get_numpy():
    ...
    try:
        ...
        raise RuntimeError(
            "Semantic retrieval requires NumPy. "
            "Install the semantic/RAG dependencies before using this feature."
        ) from error
...
def get_embedding_model():
    """
    Create the FastEmbed model only when semantic embedding/search
    is actually requested, then reuse the same instance.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "Semantic retrieval requires FastEmbed. "
            "Install the semantic/RAG dependencies before using this feature."
        ) from error
...
def create_embeddings(chunks_data):
    ...
    print(
        f"\nCreating embeddings in batches of "
        f"{BATCH_SIZE}..."
    )
