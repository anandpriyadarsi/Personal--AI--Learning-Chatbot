...
$BaseCommit = "d75a54a73a67d3e411b8104c738a72047f35c160"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "=================================================="
    Write-Host " PHASE 7.5.4 ASSESSMENTS: BLOCKED"
    Write-Host "=================================================="
    Write-Host $Message
    exit 1
}
...
if ($head -ne $BaseCommit) {
    Stop-Gate "Expected uncommitted Phase 7.5.4 work on $BaseCommit but found $head."
}
...
foreach ($item in $allowed) {
    if (-not (Test-Path $item -PathType Leaf)) {
        Stop-Gate "Missing Phase 7.5.4 file: $item"
    }
}
...
Run-Step "[1/10] Focused Phase 7.5.4 Assessments tests" {
    & $PythonResolved -m pytest tests\test_phase7_5_assessments.py -q
}

Run-Step "[2/10] Real authority-routed assessment read adapter" {
    & $PythonResolved -c "from personal_learning_assistant.services.assessment_dashboard_service import load_assessment_catalogue; d=load_assessment_catalogue(); assert d['available'] is True; assert {'summary','groups','date'} <= set(d); assert {'overdue','upcoming','completed'} == set(d['groups']); assert {'total','active','overdue','completed'} <= set(d['summary']); assert all(isinstance(d['groups'][k], list) for k in d['groups'])"
}

Run-Step "[3/10] Real read-only Assessments route smoke test" {
    & $PythonResolved -c "from personal_learning_assistant.ui.web import create_app; c=create_app({'TESTING': True}).test_client(); r=c.get('/assessments'); t=r.get_data(as_text=True); assert r.status_code == 200; assert 'Assessments' in t; assert 'Assessment timeline' in t; assert c.post('/assessments').status_code == 405; assert '<form' not in t.lower()"
}

Run-Step "[4/10] Phase 7.5.1-7.5.3 web regressions" {
    & $PythonResolved -m pytest `
        tests\test_phase7_5_web_foundation.py `
        tests\test_phase7_5_home_dashboard.py `
        tests\test_phase7_5_courses_topics.py -q
}
...
Run-Step "[6/10] Complete suite (promotion-aware)" {
    ...
    $temp = Join-Path ([IO.Path]::GetTempPath()) ("p754_phase2_" + [guid]::NewGuid().ToString("N"))
    try {
...
Run-Step "[8/10] Dependency consistency + production SQLite integrity/FK" {
    ...
    $sqliteCheckPath = Join-Path ([IO.Path]::GetTempPath()) ("p754_sqlite_" + [guid]::NewGuid().ToString("N") + ".py")
    try {
        @(
            "import sqlite3",
...
if ($migrationDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.4 must not add or modify SQLite migrations."
}
$protected = @(
...
if ($protectedDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.4 changed protected completed files: $($protectedDiff -join ', ')"
}
...
if ($staged.Count -ne 0) {
    Stop-Gate "Phase 7.5.4 gate expects no pre-staged changes."
}
try {
    git add --intent-to-add -- $allowed
    if ($LASTEXITCODE -ne 0) { Stop-Gate "Unable to mark Phase 7.5.4 files intent-to-add." }
...
foreach ($path in $trackedChanges) {
    ...
    if (-not $allowedSet.ContainsKey($normalized)) {
        Stop-Gate "Phase 7.5.4 changed an out-of-scope file: $normalized"
    }
}

if ((Get-FileHash ".\data\learning_assistant.db" -Algorithm SHA256).Hash -ne $dbBefore) {
    Stop-Gate "Phase 7.5.4 changed production SQLite."
}
if ((Get-FileHash ".\.phase4_authority.json" -Algorithm SHA256).Hash -ne $authorityBefore) {
    Stop-Gate "Phase 7.5.4 changed authority control."
}
foreach ($path in $legacyBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.4 removed legacy JSON: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $legacyBefore[$path]) {
        Stop-Gate "Phase 7.5.4 changed legacy JSON: $path"
    }
}
foreach ($path in $indexBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.4 removed retrieval-index file: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $indexBefore[$path]) {
        Stop-Gate "Phase 7.5.4 changed retrieval-index file: $path"
    }
}

Write-Host ""
Write-Host "=================================================="
Write-Host " PHASE 7.5.4 ASSESSMENTS: PASS"
Write-Host "=================================================="
Write-Host "Verified:"
Write-Host " - assessments read through the existing structured-authority repository boundary"
Write-Host " - course identity resolves through CourseService + RoutedCourseRepository"
Write-Host " - overdue, upcoming and completed assessment groups render read-only"
Write-Host " - type, status, due date, weightage and linked topics are represented safely"
Write-Host " - assessment/storage engines remain lazy at web-app creation"
Write-Host " - /assessments is GET-only and exposes no browser write form"
Write-Host " - Phase 7.5.1-7.5.3, Phase 7.1-7.4 and complete regressions pass"
...
Write-Host ""
Write-Host "Phase 7.5.4 is ready for review and commit."
