...
$Baseline = "c8e6c5720603479b8358fdd8ee37dcb535026c8b"
$Range = "$Baseline..HEAD"

function Stop-Gate { param([string]$Message)
  Write-Host ""
  Write-Host "============================================================"
  Write-Host " PHASE 7.5.15.2 VISUAL NOTES LIBRARY: BLOCKED"
  Write-Host "============================================================"
  Write-Host $Message
  exit 1
}
...
if($LASTEXITCODE -ne 0){Stop-Gate "Phase 7.5.15.1 closure commit is not an ancestor of HEAD."}
...
if($unexpected.Count){
  ...
  Stop-Gate "15.2 diff escaped approved scope."
}
...
'@
$tmp=Join-Path ([IO.Path]::GetTempPath()) ("p75152_"+[guid]::NewGuid().ToString("N")+".py")
Set-Content $tmp $VaultHashScript -Encoding UTF8
$VaultBefore=(& $Py $tmp).Trim()

Run-Step "[1/8] Phase 7.5.15.2 focused visual-library tests" {
  & $Py -m pytest -q tests/test_phase7_5_15_2_visual_notes_library.py
}

Run-Step "[2/8] Phase 7.5.15.1 canonical read-model regression" {
  & $Py -m pytest -q tests/test_phase7_5_15_1_notes_studio_read_model.py
}

Run-Step "[3/8] Phase 7.5.11 Notes compatibility regressions" {
  & $Py -m pytest -q tests/test_phase7_5_notes_resources.py tests/test_phase7_5_operational_notes_resources.py
}

Run-Step "[4/8] Obsidian workspace/reader regressions" {
  & $Py -m pytest -q tests/test_phase7_5_obsidian_workspace.py tests/test_phase7_5_obsidian_markdown_renderer.py tests/test_phase7_5_obsidian_reader_routes.py
}

Run-Step "[5/8] Academic Agent reconciled regression" {
...
Run-Step "[7/8] Compile and dependency consistency" {
...
Write-Host ""
Write-Host "[8/8] Scope and authority protections"
...
$library=Get-Content "personal_learning_assistant/services/notes_studio_library_service.py" -Raw
foreach($token in @(
  ...
  if($library.Contains($token)){Stop-Gate "Forbidden Notes Library dependency/mutation token: $token"}
}

$template=Get-Content "personal_learning_assistant/ui/web/templates/notes_library.html" -Raw
foreach($token in @("note.content","note.text","|safe")){
  if($template.Contains($token)){Stop-Gate "Visual library renders forbidden body/unsafe token: $token"}
}

Write-Host ""
Write-Host "============================================================"
Write-Host " PHASE 7.5.15.2 VISUAL NOTES LIBRARY: PASS"
Write-Host "============================================================"
Write-Host "Visual Notes Library cards, filters, responsive layout and legacy preservation are green."
