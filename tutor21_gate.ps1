...
$ExpectedBranch = "anvaya/tutor-2.1-adaptive-intelligence"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "======================================================================="
    Write-Host " ANVAYA TUTOR 2.1 ADAPTIVE INTELLIGENCE: BLOCKED"
    Write-Host "======================================================================="
    Write-Host $Message
    exit 1
}
...
if ($status.Count -ne 0) {
    ...
    Stop-Gate "Working tree must be clean before the Tutor 2.1 gate."
}
...
$HashScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor21_hash_" + [guid]::NewGuid().ToString("N") + ".py")
...
Run-Step "[1/7] Tutor 2.1 adaptive focused tests" {
    & $PythonResolved -m pytest -q tests/test_tutor21_adaptive_intelligence.py
}
...
Run-Step "[3/7] Search / reader / retrieval regressions" {
...
if ($BeforeHashes -ne $AfterHashes) {
    Stop-Gate "Tutor 2.1 tests changed production data, retrieval indexes, or configured vault Markdown."
}
...
if ($statusAfter.Count -ne 0) {
    ...
    Stop-Gate "Tutor 2.1 tests left repository changes behind."
}
...
Write-Host "======================================================================="
Write-Host "Session-local student state, adaptive quiz continuity, state-aware retrieval,"
Write-Host "and Tutor 2.0 grounded/safety behavior are green."
