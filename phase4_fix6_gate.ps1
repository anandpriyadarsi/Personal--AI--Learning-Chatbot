...
$BaseCommit = "2ca17c3238c2dbe7efc7f11d6db2c69f4de261bd"
...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "========================================="
    Write-Host " PHASE 4.6 FIX 6 GATE: BLOCKED"
    Write-Host "========================================="
    Write-Host $Message
    exit 1
}
...
foreach ($path in $requiredFiles) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Required Phase 4.6 file is missing: $path"
    }
}
...
if ($currentHead -ne $BaseCommit) {
    Stop-Gate "Phase 4.6 must remain uncommitted while gating. Expected HEAD $BaseCommit but found $currentHead."
}
...
if ($forbiddenChanges.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.6 scope is Learning Memory + Academic Progress dual-read only."
}
...
Run-Step "[1/11] Phase 4.6 focused Learning Memory + Academic Progress dual-read tests" {
    & $PythonResolved -m pytest tests\test_phase4_learning_progress_dual_read.py -q
}
Run-Step "[2/11] Phase 4.5 Attempts/Mistakes/Performance regression" {
    & $PythonResolved -m pytest tests\test_phase4_attempts_performance_dual_read.py -q
}
Run-Step "[3/11] Phase 4.4 Question/Topic Mapping regression" {
    & $PythonResolved -m pytest tests\test_phase4_question_topic_mappings_dual_read.py -q
}
Run-Step "[4/11] Phase 4.3 Questions/Sources regression" {
    & $PythonResolved -m pytest tests\test_phase4_questions_dual_read.py -q
}
Run-Step "[5/11] Phase 4.2 Assessments regression" {
    & $PythonResolved -m pytest tests\test_phase4_assessments_dual_read.py -q
}
Run-Step "[6/11] Phase 4.1 Courses/Topics regression" {
    & $PythonResolved -m pytest tests\test_phase4_courses_dual_read.py -q
}
Run-Step "[7/11] All Phase 3 regression/safety tests" {
    & $PythonResolved -m pytest $phase3Tests -q
}
Run-Step "[8/11] Full regression suite" {
    & $PythonResolved -m pytest -q
}
Run-Step "[9/11] Python compilation" {
    & $PythonResolved -m compileall -q .
}
Run-Step "[10/11] Installed dependency consistency" {
    & $PythonResolved -m pip check
}

Write-Host ""
Write-Host "[11/11] Git whitespace/error check and repository hygiene"
...
if ($stagedBefore.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.6 gate expects no pre-staged changes so new files can be validated safely."
}
...
if ($intentAddFailed) { Stop-Gate "Unable to mark Phase 4.6 files intent-to-add for git diff --check." }
if ($diffCheckFailed) { Stop-Gate "git diff --check found whitespace/errors in the Phase 4.6 change set." }
...
Write-Host ""
Write-Host "========================================="
Write-Host " PHASE 4.6 FIX 6 GATE: PASS"
Write-Host "========================================="
Write-Host "Verified:"
Write-Host "  - baseline HEAD remains 2ca17c3 and branch is phase4/structured-cutover"
Write-Host "  - learning_memory.py, academic_progress.py, planners, dashboards, and agents are unchanged"
Write-Host "  - legacy learning_memory.json and course_progress_history.json remain authoritative"
Write-Host "  - SQLite learning_memory_entries/topic_progress_events/progress_snapshots are read-only shadow state"
Write-Host "  - Learning Memory and Academic Progress remain distinct domains"
Write-Host "  - weak/mastered memory evidence does not overwrite current topic status"
Write-Host "  - current topic status/confidence and course progress summaries are compared without writing history"
Write-Host "  - persisted historical progress snapshots are compared without regeneration"
Write-Host "  - unresolved/global memory topics remain explicit raw/deferred evidence"
Write-Host "  - ledger targets and actual course/topic/snapshot relationships are structurally validated"
Write-Host "  - parity reads create no progress snapshot, memory entry, or activity event"
...
Write-Host "  - Phase 4.5 through Phase 4.1, Phase 3, and full regression suites pass"
