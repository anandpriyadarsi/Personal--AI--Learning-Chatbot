...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "======================================================================="
    Write-Host " ANVAYA TUTOR 2.3.3 SOCRATIC MULTI-TURN LOOP: BLOCKED"
    Write-Host "======================================================================="
    Write-Host $Message
    exit 1
}
...
if ($status.Count -ne 0) {
    ...
    Stop-Gate "Working tree must be clean before the Tutor 2.3.3 gate."
}
...
$HashScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix3_hash_" + [guid]::NewGuid().ToString("N") + ".py")
...
function Show-ProtectedHashDiff {
    ...
    $BeforePath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix3_before_" + [guid]::NewGuid().ToString("N") + ".json")
    $AfterPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix3_after_" + [guid]::NewGuid().ToString("N") + ".json")
...
'@
    ...
    $DiffScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix3_diff_" + [guid]::NewGuid().ToString("N") + ".py")
...
Run-Step "[1/11] Tutor 2.3.3 Socratic multi-turn focused tests" {
    & $PythonResolved -m pytest -q tests/test_tutor23_socratic_loop.py
}
Assert-ProtectedHashesUnchanged "[1/11] Tutor 2.3.3 focused tests"


Run-Step "[2/11] Tutor 2.3.2 diagnostic-questioning regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_diagnostic_questioning.py
}
Assert-ProtectedHashesUnchanged "[2/11] Tutor 2.3.2 regressions"


Run-Step "[3/11] Tutor 2.3.1 session-goal + teaching-plan regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_session_goal_plan.py
}
Assert-ProtectedHashesUnchanged "[3/11] Tutor 2.3.1 regressions"


Run-Step "[4/11] Tutor 2.2 student-model + signals + memory + personalized-policy regressions" {
...
Assert-ProtectedHashesUnchanged "[4/11] Tutor 2.2 regressions"


Run-Step "[5/11] Tutor 2.1 adaptive + correctness + retrieval + live-fix regressions" {
...
Assert-ProtectedHashesUnchanged "[5/11] Tutor 2.1 regressions"


Run-Step "[6/11] Tutor 2.0 foundation + grounded Tutor regressions" {
...
Assert-ProtectedHashesUnchanged "[6/11] Tutor 2.0/web regressions"


Run-Step "[7/11] Search / reader / retrieval regressions" {
...
Assert-ProtectedHashesUnchanged "[7/11] Search/reader/retrieval regressions"


Run-Step "[8/11] Complete pytest regression" {
...
Assert-ProtectedHashesUnchanged "[8/11] Complete pytest regression"


Run-Step "[9/11] Compile + dependency check" {
    & $PythonResolved -m compileall -q personal_learning_assistant tests
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $PythonResolved -m pip check
}
Assert-ProtectedHashesUnchanged "[9/11] Compile/dependency check"


Write-Host ""
Write-Host "[10/11] Protected production/index/vault hashes"
...
if ($BeforeHashes -ne $AfterHashes) {
    ...
    Stop-Gate "Tutor 2.3.3 tests changed production data, learning memory, progress, retrieval indexes, or configured vault Markdown."
}
...
if ($statusAfter.Count -ne 0) {
    ...
    Stop-Gate "Tutor 2.3.3 tests left repository changes behind."
}
...
Write-Host ""
Write-Host "======================================================================="
Write-Host " ANVAYA TUTOR 2.3.3 SOCRATIC MULTI-TURN LOOP: PASS"
Write-Host "======================================================================="
Write-Host "Pending-question continuity, diagnostic-answer evaluation, Socratic outcome state, and student override are green."
Write-Host "Tutor 2.3.2 diagnostics, Tutor 2.3.1 session-goal/teaching-plan behavior, Tutor 2.2 memory/personalization,"
Write-Host "Tutor 2.1 adaptive/correctness/retrieval behavior, provider-output hygiene, and Tutor 2.0 safety remain green."
