...
$BaseCommit = "2f04768c5e5da72b414bc138acf1e3ac107de99b"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "============================================"
    Write-Host " PHASE 6.3 KNOWLEDGE NAVIGATOR: BLOCKED"
    Write-Host "============================================"
    Write-Host $Message
    exit 1
}
...
if ($head -ne $BaseCommit) {
    Stop-Gate "Expected uncommitted Phase 6.3 work on $BaseCommit but found $head."
}

$allowed = @(
    "personal_learning_assistant/domain/knowledge_navigator_models.py",
    "personal_learning_assistant/repositories/sqlite/knowledge_navigator_repository.py",
    "personal_learning_assistant/services/knowledge_navigator_service.py",
    "phase6_knowledge_navigator.py",
    "tests/test_phase6_knowledge_navigator.py",
    "PHASE6_FIX3_KNOWLEDGE_NAVIGATOR.md",
    "phase6_fix3_gate.ps1"
)
$allowedSet = @{}
foreach ($item in $allowed) {
    $allowedSet[$item] = $true
    if (-not (Test-Path $item -PathType Leaf)) {
        Stop-Gate "Missing Phase 6.3 file: $item"
    }
}
...
Run-Step "[1/9] Focused Phase 6.3 Knowledge Navigator tests" {
    & $PythonResolved -m pytest tests\test_phase6_knowledge_navigator.py -q
}

Write-Host ""
Write-Host "[2/9] REAL MA103N Knowledge Navigator preview (read-only)"
& $PythonResolved .\phase6_knowledge_navigator.py `
    ...
    --limit-sources 5
if ($LASTEXITCODE -ne 0) {
    Stop-Gate "Real MA103N Knowledge Navigator preview failed."
}

Run-Step "[3/9] Phase 6.1-6.2 regressions" {
    & $PythonResolved -m pytest `
        tests\test_phase6_tutor_foundation.py `
        tests\test_phase6_grounded_tutor_engine.py -q
}
...
Run-Step "[5/9] Complete suite (promotion-aware)" {
    ...
    $temp = Join-Path ([IO.Path]::GetTempPath()) ("p63_phase2_" + [guid]::NewGuid().ToString("N"))
    try {
...
Run-Step "[8/9] Production SQLite integrity/FK read-only" {
    $sqliteCheckPath = Join-Path ([IO.Path]::GetTempPath()) ("p63_sqlite_" + [guid]::NewGuid().ToString("N") + ".py")
    try {
        @(
            "import sqlite3",
...
if ($staged.Count -ne 0) {
    Stop-Gate "Phase 6.3 gate expects no pre-staged changes."
}
try {
    git add --intent-to-add -- $allowed
    if ($LASTEXITCODE -ne 0) {
        Stop-Gate "Unable to mark Phase 6.3 files intent-to-add."
    }
...
foreach ($file in $currentIndexFiles) {
    ...
    if ((Get-FileHash $file.FullName -Algorithm SHA256).Hash -ne $indexBefore[$file.FullName]) {
        Stop-Gate "Phase 6.3 gate changed retrieval index bytes."
    }
}
...
if ($migrationDiff.Count -ne 0) {
    Stop-Gate "Phase 6.3 must not add or modify a SQLite migration."
}

Write-Host ""
Write-Host "============================================"
Write-Host " PHASE 6.3 KNOWLEDGE NAVIGATOR: PASS"
Write-Host "============================================"
Write-Host "Verified:"
Write-Host " - topic ordering is deterministic and based on structured evidence"
Write-Host " - lower confidence, active deadlines and unresolved mistakes can raise priority"
Write-Host " - learning memory, plan coverage and study time are reported without opaque inference"
Write-Host " - direct topic sources outrank weaker course-only relationships"
Write-Host " - current retrievable chunks and unfinished resource progress influence source ordering"
Write-Host " - notes and resources are surfaced with human-readable reasons"
Write-Host " - completed resources are deprioritized rather than silently hidden"
Write-Host " - real MA103N preview performs zero SQLite/provider/network writes"
Write-Host " - no study-plan, progress, memory, assessment or resource state is modified"
Write-Host " - Phase 6.1-6.2, Phase 5 and complete project regressions pass"
Write-Host " - no SQLite migration is introduced in Phase 6.3"
Write-Host ""
Write-Host "Phase 6.3 is ready for review and commit."
