...
$BaseCommit = "c61c568d937b8bf7d50e44267e6e9d6ba24a8326"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "=================================================="
    Write-Host " PHASE 7.5.2 HOME / ACADEMIC BRIEF: BLOCKED"
    Write-Host "=================================================="
    Write-Host $Message
    exit 1
}
...
if ($head -ne $BaseCommit) {
    Stop-Gate "Expected uncommitted Phase 7.5.2 work on $BaseCommit but found $head."
}
...
foreach ($item in $allowed) {
    if (-not (Test-Path $item -PathType Leaf)) {
        Stop-Gate "Missing Phase 7.5.2 file: $item"
    }
}
...
Run-Step "[1/10] Focused Phase 7.5.2 Home dashboard tests" {
    & $PythonResolved -m pytest tests\test_phase7_5_home_dashboard.py -q
}

Run-Step "[2/10] Real Daily Academic Brief read-model adapter" {
    & $PythonResolved -c "from personal_learning_assistant.services.home_dashboard_service import load_home_dashboard; d=load_home_dashboard(); assert d['available'] is True; assert {'summary','deadlines','study_blocks','priorities','risks','best_next_action'} <= set(d); assert isinstance(d['deadlines'], list); assert isinstance(d['study_blocks'], list); assert isinstance(d['priorities'], list); assert isinstance(d['risks'], list)"
}

Run-Step "[3/10] Real read-only Home route smoke test" {
    & $PythonResolved -c "from personal_learning_assistant.ui.web import create_app; c=create_app({'TESTING': True}).test_client(); r=c.get('/'); t=r.get_data(as_text=True); assert r.status_code == 200; assert 'Today at a glance' in t; assert 'Best next action' in t; assert 'Deadline alerts' in t; assert c.post('/').status_code == 405"
}

Run-Step "[4/10] Phase 7.5.1 web-foundation regressions" {
...
Run-Step "[6/10] Complete suite (promotion-aware)" {
    ...
    $temp = Join-Path ([IO.Path]::GetTempPath()) ("p752_phase2_" + [guid]::NewGuid().ToString("N"))
    try {
...
Run-Step "[8/10] Dependency consistency + production SQLite integrity/FK" {
    ...
    $sqliteCheckPath = Join-Path ([IO.Path]::GetTempPath()) ("p752_sqlite_" + [guid]::NewGuid().ToString("N") + ".py")
    try {
        @(
            "import sqlite3",
...
Write-Host ""
Write-Host "[9/10] Scope + completed Phase 7 immutability"
...
if ($migrationDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.2 must not add or modify SQLite migrations."
}
$protected = @(
...
if ($protectedDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.2 changed protected completed files: $($protectedDiff -join ', ')"
}
...
if ($staged.Count -ne 0) {
    Stop-Gate "Phase 7.5.2 gate expects no pre-staged changes."
}
try {
    git add --intent-to-add -- $allowed
    if ($LASTEXITCODE -ne 0) { Stop-Gate "Unable to mark Phase 7.5.2 files intent-to-add." }
...
foreach ($path in $trackedChanges) {
    ...
    if (-not $allowedSet.ContainsKey($normalized)) {
        Stop-Gate "Phase 7.5.2 changed an out-of-scope file: $normalized"
    }
}

if ((Get-FileHash ".\data\learning_assistant.db" -Algorithm SHA256).Hash -ne $dbBefore) {
    Stop-Gate "Phase 7.5.2 changed production SQLite."
}
if ((Get-FileHash ".\.phase4_authority.json" -Algorithm SHA256).Hash -ne $authorityBefore) {
    Stop-Gate "Phase 7.5.2 changed authority control."
}
foreach ($path in $legacyBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.2 removed legacy JSON: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $legacyBefore[$path]) {
        Stop-Gate "Phase 7.5.2 changed legacy JSON: $path"
    }
}
foreach ($path in $indexBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.2 removed retrieval-index file: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $indexBefore[$path]) {
        Stop-Gate "Phase 7.5.2 changed retrieval-index file: $path"
    }
}

Write-Host ""
Write-Host "=================================================="
Write-Host " PHASE 7.5.2 HOME / ACADEMIC BRIEF: PASS"
Write-Host "=================================================="
Write-Host "Verified:"
Write-Host " - Home reads the existing Daily Academic Brief through a service adapter"
Write-Host " - deadlines, study blocks, priorities, risk and best-next-action render read-only"
Write-Host " - empty and unavailable states are safe and do not expose exception content"
Write-Host " - Daily Brief / planner / optional AI modules remain lazy at web-app creation"
Write-Host " - Phase 7.5.1, Phase 7.1-7.4 and complete regressions pass"
...
Write-Host ""
Write-Host "Phase 7.5.2 is ready for review and commit."
