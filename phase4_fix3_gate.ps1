...
$BaseCommit = "506b9c3093e6fdb6373fb77a370ef1ef9cba9d09"
...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "========================================="
    Write-Host " PHASE 4.3 FIX 3 GATE: BLOCKED"
    Write-Host "========================================="
    Write-Host $Message
    exit 1
}
...
$requiredFiles = @(
    ...
    "tests\fixtures\phase4\assessment_workspace.json",
...
foreach ($path in $requiredFiles) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Required Phase 4.3 file is missing: $path"
    }
}
...
if ($currentHead -ne $BaseCommit) {
    Stop-Gate "Phase 4.3 must remain uncommitted while gating. Expected HEAD $BaseCommit but found $currentHead."
}
...
if ($forbiddenChanges.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.3 scope is Assessment Questions + Question Sources dual-read only."
}
...
Run-Step "[1/8] Phase 4.3 focused Questions/Sources dual-read tests" {
    & $PythonResolved -m pytest tests\test_phase4_questions_dual_read.py -q
}

Run-Step "[2/8] Phase 4.2 Assessments regression" {
    & $PythonResolved -m pytest tests\test_phase4_assessments_dual_read.py -q
}

Run-Step "[3/8] Phase 4.1 Courses/Topics regression" {
    & $PythonResolved -m pytest tests\test_phase4_courses_dual_read.py -q
}

Run-Step "[4/8] All Phase 3 regression/safety tests" {
    & $PythonResolved -m pytest $phase3Tests -q
}

Run-Step "[5/8] Full regression suite" {
...
if ($stagedBefore.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.3 gate expects no pre-staged changes so new files can be validated safely."
}
...
if ($intentAddFailed) {
    Stop-Gate "Unable to mark Phase 4.3 files intent-to-add for git diff --check."
}
if ($diffCheckFailed) {
    Stop-Gate "git diff --check found whitespace/errors in the Phase 4.3 change set."
}
...
Write-Host ""
Write-Host "========================================="
Write-Host " PHASE 4.3 FIX 3 GATE: PASS"
Write-Host "========================================="
Write-Host "Verified:"
Write-Host "  - baseline HEAD remains 506b9c3 and branch is phase4/structured-cutover"
Write-Host "  - assessment_question_workspace.py public API/writers are unchanged"
Write-Host "  - legacy assessment_workspace.json remains authoritative for reads/writes"
Write-Host "  - SQLite Questions/Question Sources is read-only shadow state"
Write-Host "  - actual assessment/source ownership and meaningful ordering are validated"
Write-Host "  - raw question/source/status evidence is compared without silent normalization"
Write-Host "  - reviewed document/resource/note targets are structurally validated and deferred"
Write-Host "  - question-topic mappings remain deferred to Phase 4.4"
Write-Host "  - attempts/mistakes/performance remain outside Phase 4.3"
Write-Host "  - integrity_check and foreign_key_check are exercised by focused tests"
Write-Host "  - Phase 4.2, Phase 4.1, Phase 3, and full regression suites pass"
