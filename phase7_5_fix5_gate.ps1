...
$BaseCommit = "e5895e153e9f401518b33bce29ba23b473009e2e"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "=================================================="
    Write-Host " PHASE 7.5.5 PROGRESS / PLANNING: BLOCKED"
    Write-Host "=================================================="
    Write-Host $Message
    exit 1
}
...
if ($head -ne $BaseCommit) {
    Stop-Gate "Expected uncommitted Phase 7.5.5 work on $BaseCommit but found $head."
}
...
foreach ($item in $allowed) {
    if (-not (Test-Path $item -PathType Leaf)) {
        Stop-Gate "Missing Phase 7.5.5 file: $item"
    }
}
...
Run-Step "[1/10] Focused Phase 7.5.5 Progress & Planning tests" {
    & $PythonResolved -m pytest tests\test_phase7_5_progress_planning.py -q
}

Run-Step "[2/10] Real authority-routed progress + saved-plan read adapter" {
    & $PythonResolved -c "from personal_learning_assistant.services.planning_dashboard_service import load_planning_dashboard; d=load_planning_dashboard(); assert d['available'] is True; assert {'summary','courses','plans'} <= set(d); assert {'courses','average_progress','mastered_topics','weak_topics','missing_topics','saved_plans'} <= set(d['summary']); assert len(d['plans']) == 3; assert all(p['key'] in {'weekly','multi','intelligent'} for p in d['plans'])"
}

Run-Step "[3/10] Real read-only Planning route smoke test" {
    & $PythonResolved -c "from personal_learning_assistant.ui.web import create_app; c=create_app({'TESTING': True}).test_client(); r=c.get('/planning'); t=r.get_data(as_text=True); assert r.status_code == 200; assert 'Progress &amp; planning' in t; assert c.post('/planning').status_code == 405; assert '<form' not in t.lower()"
}

Run-Step "[4/10] Phase 7.5.1-7.5.4 web regressions" {
    & $PythonResolved -m pytest `
        ...
        tests\test_phase7_5_assessments.py -q
}
...
Run-Step "[6/10] Complete suite (promotion-aware)" {
    ...
    $temp = Join-Path ([IO.Path]::GetTempPath()) ("p755_phase2_" + [guid]::NewGuid().ToString("N"))
    try {
...
Run-Step "[8/10] Dependency consistency + production SQLite integrity/FK" {
    ...
    $sqliteCheckPath = Join-Path ([IO.Path]::GetTempPath()) ("p755_sqlite_" + [guid]::NewGuid().ToString("N") + ".py")
    try {
        @(
            "import sqlite3",
...
if ($migrationDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.5 must not add or modify SQLite migrations."
}
$protected = @(
...
if ($protectedDiff.Count -ne 0) {
    Stop-Gate "Phase 7.5.5 changed protected completed files: $($protectedDiff -join ', ')"
}
...
if ($staged.Count -ne 0) {
    Stop-Gate "Phase 7.5.5 gate expects no pre-staged changes."
}
try {
    git add --intent-to-add -- $allowed
    if ($LASTEXITCODE -ne 0) { Stop-Gate "Unable to mark Phase 7.5.5 files intent-to-add." }
...
foreach ($path in $trackedChanges) {
    ...
    if (-not $allowedSet.ContainsKey($normalized)) {
        Stop-Gate "Phase 7.5.5 changed an out-of-scope file: $normalized"
    }
}

if ((Get-FileHash ".\data\learning_assistant.db" -Algorithm SHA256).Hash -ne $dbBefore) {
    Stop-Gate "Phase 7.5.5 changed production SQLite."
}
if ((Get-FileHash ".\.phase4_authority.json" -Algorithm SHA256).Hash -ne $authorityBefore) {
    Stop-Gate "Phase 7.5.5 changed authority control."
}
foreach ($path in $legacyBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.5 removed legacy JSON: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $legacyBefore[$path]) {
        Stop-Gate "Phase 7.5.5 changed legacy JSON: $path"
    }
}
foreach ($path in $indexBefore.Keys) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Phase 7.5.5 removed retrieval-index file: $path"
    }
    if ((Get-FileHash $path -Algorithm SHA256).Hash -ne $indexBefore[$path]) {
        Stop-Gate "Phase 7.5.5 changed retrieval-index file: $path"
    }
}

Write-Host ""
Write-Host "=================================================="
Write-Host " PHASE 7.5.5 PROGRESS / PLANNING: PASS"
Write-Host "=================================================="
Write-Host "Verified:"
Write-Host " - progress reads use existing course/progress boundaries without recording snapshots"
Write-Host " - latest weekly, multi-course and intelligent saved plans render read-only"
Write-Host " - priority topics, gaps, confidence and progress deltas are represented safely"
Write-Host " - progress/planner engines remain lazy at web-app creation"
Write-Host " - /planning is GET-only and exposes no browser write form"
Write-Host " - Phase 7.5.1-7.5.4, Phase 7.1-7.4 and complete regressions pass"
Write-Host " - production SQLite, authority, legacy JSON and retrieval index remain unchanged"
Write-Host " - no snapshot recording, plan generation, migration, deletion or authority switch occurs"
Write-Host ""
Write-Host "Phase 7.5.5 is ready for review and commit."
