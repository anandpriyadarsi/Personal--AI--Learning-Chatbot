...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "======================================================================="
    Write-Host " ANVAYA TUTOR 2.3.6 UNDERSTANDING / EXIT CHECK: BLOCKED"
    Write-Host "======================================================================="
    Write-Host $Message
    exit 1
}
...
if ($status.Count -ne 0) {
    ...
    Stop-Gate "Working tree must be clean before the Tutor 2.3.6 gate."
}
...
$HashScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix6_hash_" + [guid]::NewGuid().ToString("N") + ".py")
...
function Show-ProtectedHashDiff {
    ...
    $BeforePath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix6_before_" + [guid]::NewGuid().ToString("N") + ".json")
    $AfterPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix6_after_" + [guid]::NewGuid().ToString("N") + ".json")
...
'@
    ...
    $DiffScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix6_diff_" + [guid]::NewGuid().ToString("N") + ".py")
...
Run-Step "[1/14] Tutor 2.3.6 understanding/exit-check focused tests" {
    & $PythonResolved -m pytest -q tests/test_tutor23_goal_exit_check.py
}
Assert-ProtectedHashesUnchanged "[1/14] Tutor 2.3.6 focused tests"


Run-Step "[2/14] Tutor 2.3.5 adaptive-practice regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_adaptive_practice.py
}
Assert-ProtectedHashesUnchanged "[2/14] Tutor 2.3.5 regressions"


Run-Step "[3/14] Tutor 2.3.4 prerequisite-reasoning regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_prerequisite_reasoning.py
}
Assert-ProtectedHashesUnchanged "[3/14] Tutor 2.3.4 regressions"


Run-Step "[4/14] Tutor 2.3.3 Socratic multi-turn regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_socratic_loop.py
}
Assert-ProtectedHashesUnchanged "[4/14] Tutor 2.3.3 regressions"


Run-Step "[5/14] Tutor 2.3.2 diagnostic-questioning regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_diagnostic_questioning.py
}
Assert-ProtectedHashesUnchanged "[5/14] Tutor 2.3.2 regressions"


Run-Step "[6/14] Tutor 2.3.1 session-goal + teaching-plan regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_session_goal_plan.py
}
Assert-ProtectedHashesUnchanged "[6/14] Tutor 2.3.1 regressions"


Run-Step "[7/14] Tutor 2.2 student-model + signals + memory + personalized-policy regressions" {
...
Assert-ProtectedHashesUnchanged "[7/14] Tutor 2.2 regressions"


Run-Step "[8/14] Tutor 2.1 adaptive + correctness + retrieval + live-fix regressions" {
...
Assert-ProtectedHashesUnchanged "[8/14] Tutor 2.1 regressions"


Run-Step "[9/14] Tutor 2.0 foundation + grounded Tutor regressions" {
...
Assert-ProtectedHashesUnchanged "[9/14] Tutor 2.0/web regressions"


Run-Step "[10/14] Search / reader / retrieval regressions" {
...
Assert-ProtectedHashesUnchanged "[10/14] Search/reader/retrieval regressions"


Run-Step "[11/14] Complete pytest regression" {
...
Assert-ProtectedHashesUnchanged "[11/14] Complete pytest regression"


Run-Step "[12/14] Compile + dependency check" {
    & $PythonResolved -m compileall -q personal_learning_assistant tests
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $PythonResolved -m pip check
}
Assert-ProtectedHashesUnchanged "[12/14] Compile/dependency check"


Write-Host ""
Write-Host "[13/14] Protected production/index/vault hashes"
...
if ($BeforeHashes -ne $AfterHashes) {
    ...
    Stop-Gate "Tutor 2.3.6 tests changed production data, learning memory, progress, retrieval indexes, or configured vault Markdown."
}

Write-Host ""
Write-Host "[14/14] Repository hygiene"
git diff --check
...
if ($statusAfter.Count -ne 0) {
    ...
    Stop-Gate "Tutor 2.3.6 tests left repository changes behind."
}
...
Write-Host ""
Write-Host "======================================================================="
Write-Host " ANVAYA TUTOR 2.3.6 UNDERSTANDING / EXIT CHECK: PASS"
Write-Host "======================================================================="
Write-Host "One-shot exit checks, current-session goal evidence, likely-met/unresolved transitions, and self-report safeguards are green."
Write-Host "Tutor 2.3.5 adaptive practice, Tutor 2.3.4 prerequisite reasoning, Tutor 2.3.3 Socratic continuity, Tutor 2.3.2 diagnostics, Tutor 2.3.1 session-goal/teaching-plan behavior,"
