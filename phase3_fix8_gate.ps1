...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "========================================="
    Write-Host " PHASE 3.1 FIX 8 GATE: BLOCKED"
    Write-Host "========================================="
    Write-Host $Message
    exit 1
}
...
Run-Step "[1/5] Phase 3.1 Fix 8 attempts/mistakes/performance tests" {
    & $PythonResolved -m pytest `
        tests\test_phase3_attempts_performance_importer.py `
        -q
}
...
Write-Host ""
Write-Host "========================================="
Write-Host " PHASE 3.1 FIX 8 GATE: PASS"
Write-Host "========================================="
Write-Host "Verified:"
Write-Host "  - attempt evidence imports after exact question-hash prerequisites"
Write-Host "  - mistake rows are created only from attempt-attached mistake text"
Write-Host "  - standalone mistakes remain deferred/reviewable instead of guessed"
Write-Host "  - identical re-import is idempotent"
Write-Host "  - changed source hashes update stable targets and record new evidence"
Write-Host "  - invalid outcomes/marks do not break SQLite constraints"
