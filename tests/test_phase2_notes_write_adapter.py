...
def test_service_create_note_is_non_interactive_and_preserves_values(
    tmp_path,
    monkeypatch,
):
...
def test_repository_append_keeps_existing_notes_and_v1_shape(
    tmp_path,
):
    path = tmp_path / "notes.json"
    path.write_text(
...
def test_repository_invalid_legacy_json_matches_old_add_behavior(
    tmp_path,
):
    path = tmp_path / "notes.json"
    path.write_text(
...
def test_read_queries_still_leave_file_hash_unchanged(
    tmp_path,
):
    path = tmp_path / "notes.json"
    path.write_text(
...
def test_root_add_note_delegates_to_extracted_cli(
    monkeypatch,
):
    calls = []
