...
$BaseCommit = "b7bd07a0572a88c8063ccde8bb414f569574473f"

function Stop-Gate {
    param([string]$Message)
    Write-Host ""
    Write-Host "============================================"
    Write-Host " PHASE 6.4 LECTURE LEARNING MODE: BLOCKED"
    Write-Host "============================================"
    Write-Host $Message
    exit 1
}
...
if ($head -ne $BaseCommit) {
    Stop-Gate "Expected uncommitted Phase 6.4 work on $BaseCommit but found $head."
}

$allowed = @(
    "personal_learning_assistant/domain/lecture_learning_models.py",
    "personal_learning_assistant/repositories/sqlite/lecture_learning_repository.py",
    "personal_learning_assistant/services/lecture_learning_service.py",
    "phase6_lecture_learning.py",
    "tests/test_phase6_lecture_learning_mode.py",
    "PHASE6_FIX4_LECTURE_LEARNING_MODE.md",
    "phase6_fix4_gate.ps1"
)
$allowedSet = @{}
foreach ($item in $allowed) {
    $allowedSet[$item] = $true
    if (-not (Test-Path $item -PathType Leaf)) {
        Stop-Gate "Missing Phase 6.4 file: $item"
    }
}
...
Run-Step "[1/9] Focused Phase 6.4 Lecture Learning tests" {
    & $PythonResolved -m pytest tests\test_phase6_lecture_learning_mode.py -q
}

Write-Host ""
Write-Host "[2/9] REAL MA103N MIT lecture listing (read-only)"
& $PythonResolved .\phase6_lecture_learning.py `
    ...
    --limit 5
if ($LASTEXITCODE -ne 0) {
    Stop-Gate "Real MA103N lecture preview failed."
}

Run-Step "[3/9] Phase 6.1-6.3 regressions" {
    & $PythonResolved -m pytest `
        tests\test_phase6_tutor_foundation.py `
        tests\test_phase6_grounded_tutor_engine.py `
        tests\test_phase6_knowledge_navigator.py -q
}

Run-Step "[4/9] Phase 5 Resources/retrieval/closure regressions" {
    & $PythonResolved -m pytest `
...
Run-Step "[5/9] Complete suite (promotion-aware)" {
    ...
    $temp = Join-Path ([IO.Path]::GetTempPath()) ("p64_phase2_" + [guid]::NewGuid().ToString("N"))
    try {
...
Run-Step "[8/9] Production SQLite integrity/FK read-only" {
    $sqliteCheckPath = Join-Path ([IO.Path]::GetTempPath()) ("p64_sqlite_" + [guid]::NewGuid().ToString("N") + ".py")
    try {
        @(
            "import sqlite3",
...
if ($staged.Count -ne 0) {
    Stop-Gate "Phase 6.4 gate expects no pre-staged changes."
}
try {
    git add --intent-to-add -- $allowed
    if ($LASTEXITCODE -ne 0) {
        Stop-Gate "Unable to mark Phase 6.4 files intent-to-add."
    }
...
foreach ($file in $currentIndexFiles) {
    ...
    if ((Get-FileHash $file.FullName -Algorithm SHA256).Hash -ne $indexBefore[$file.FullName]) {
        Stop-Gate "Phase 6.4 gate changed retrieval index bytes."
    }
}
...
if ($migrationDiff.Count -ne 0) {
    Stop-Gate "Phase 6.4 must not add or modify a SQLite migration."
}

Write-Host ""
Write-Host "============================================"
Write-Host " PHASE 6.4 LECTURE LEARNING MODE: PASS"
Write-Host "============================================"
Write-Host "Verified:"
Write-Host " - existing resource_progress_events + study_sessions are reused; no parallel time store"
Write-Host " - start/checkpoint/pause/resume/finish are explicit event-driven operations"
Write-Host " - elapsed study minutes come only from explicit start/resume and pause/finish timestamps"
Write-Host " - player position/progress is preserved only from explicit supplied values"
Write-Host " - each resume opens a new study-session segment; prior segments remain history"
Write-Host " - ambiguous multi-topic lectures are never silently assigned to one topic"
Write-Host " - resource status/progress and study-session writes are transactional with outbox events"
Write-Host " - write operations require active SQLite authority"
Write-Host " - lecture-specific tutor sessions reuse Phase 6.1 mode=lecture + exact resource scope"
Write-Host " - real MA103N MIT preview performs zero SQLite/provider/network writes"
Write-Host " - no mastery, memory, study-plan, assessment or knowledge state is changed"
Write-Host " - Phase 6.1-6.3, Phase 5 and complete project regressions pass"
Write-Host " - no SQLite migration is introduced in Phase 6.4"
Write-Host ""
Write-Host "Phase 6.4 is ready for review and commit."
