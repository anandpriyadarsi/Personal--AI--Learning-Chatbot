...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "========================================="
    Write-Host " PHASE 4.4 FIX 4 GATE: BLOCKED"
    Write-Host "========================================="
    Write-Host $Message
    exit 1
}
...
$requiredFiles = @(
    ...
    "tests\fixtures\phase4\question_topic_mappings_workspace.json",
...
foreach ($path in $requiredFiles) {
    if (-not (Test-Path $path -PathType Leaf)) {
        Stop-Gate "Required Phase 4.4 file is missing: $path"
    }
}
...
if ($currentHead -ne $BaseCommit) {
    Stop-Gate "Phase 4.4 must remain uncommitted while gating. Expected HEAD $BaseCommit but found $currentHead."
}
...
if ($forbiddenChanges.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.4 scope is Question <-> Topic Mapping dual-read only."
}
...
Write-Host ""
Write-Host "[9/9] Git whitespace/error check and repository hygiene"
...
if ($stagedBefore.Count -ne 0) {
    ...
    Stop-Gate "Phase 4.4 gate expects no pre-staged changes so new files can be validated safely."
}
...
if ($intentAddFailed) { Stop-Gate "Unable to mark Phase 4.4 files intent-to-add for git diff --check." }
if ($diffCheckFailed) { Stop-Gate "git diff --check found whitespace/errors in the Phase 4.4 change set." }
...
Write-Host ""
Write-Host "========================================="
Write-Host " PHASE 4.4 FIX 4 GATE: PASS"
Write-Host "========================================="
Write-Host "Verified:"
Write-Host "  - baseline HEAD remains eb53aee and branch is phase4/structured-cutover"
Write-Host "  - automatic_topic_mapping.py and assessment_question_workspace.py are unchanged"
Write-Host "  - legacy assessment_workspace.json remains authoritative for mapping reads/writes"
Write-Host "  - SQLite question_topic_mappings is read-only shadow state"
Write-Host "  - stored decisions are compared without rerunning the heuristic mapper"
Write-Host "  - accepted/proposed state, rank, score, method, reason, and timestamps are compared"
Write-Host "  - actual question/topic/course relationships are structurally validated"
Write-Host "  - unresolved candidates remain deferred; historical rows remain history"
Write-Host "  - attempts/mistakes/performance remain outside Phase 4.4"
Write-Host "  - integrity_check and foreign_key_check are exercised by focused tests"
Write-Host "  - Phase 4.3, Phase 4.2, Phase 4.1, Phase 3, and full suites pass"
