...
function Stop-Gate {
    ...
    Write-Host ""
    Write-Host "======================================================================="
    Write-Host " ANVAYA TUTOR 2.3.2 DIAGNOSTIC QUESTIONING: BLOCKED"
    Write-Host "======================================================================="
    Write-Host $Message
    exit 1
}
...
if ($status.Count -ne 0) {
    ...
    Stop-Gate "Working tree must be clean before the Tutor 2.3.2 gate."
}
...
$HashScriptPath = Join-Path ([IO.Path]::GetTempPath()) ("anvaya_tutor23_fix2_hash_" + [guid]::NewGuid().ToString("N") + ".py")
...
Run-Step "[1/10] Tutor 2.3.2 diagnostic-questioning focused tests" {
    & $PythonResolved -m pytest -q tests/test_tutor23_diagnostic_questioning.py
}
Assert-ProtectedHashesUnchanged "[1/10] Tutor 2.3.2 focused tests"


Run-Step "[2/10] Tutor 2.3.1 session-goal + teaching-plan regressions" {
    & $PythonResolved -m pytest -q tests/test_tutor23_session_goal_plan.py
}
Assert-ProtectedHashesUnchanged "[2/10] Tutor 2.3.1 regressions"


Run-Step "[3/10] Tutor 2.2 student-model + signals + memory + personalized-policy regressions" {
...
Assert-ProtectedHashesUnchanged "[3/10] Tutor 2.2 regressions"


Run-Step "[4/10] Tutor 2.1 adaptive + correctness + retrieval + live-fix regressions" {
...
Assert-ProtectedHashesUnchanged "[4/10] Tutor 2.1 regressions"


Run-Step "[5/10] Tutor 2.0 foundation + grounded Tutor regressions" {
...
Assert-ProtectedHashesUnchanged "[5/10] Tutor 2.0/web regressions"


Run-Step "[6/10] Search / reader / retrieval regressions" {
...
Assert-ProtectedHashesUnchanged "[6/10] Search/reader/retrieval regressions"


Run-Step "[7/10] Complete pytest regression" {
...
Assert-ProtectedHashesUnchanged "[7/10] Complete pytest regression"


Run-Step "[8/10] Compile + dependency check" {
    & $PythonResolved -m compileall -q personal_learning_assistant tests
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $PythonResolved -m pip check
}
Assert-ProtectedHashesUnchanged "[8/10] Compile/dependency check"


Write-Host ""
Write-Host "[9/10] Protected production/index/vault hashes"
...
if ($BeforeHashes -ne $AfterHashes) {
    ...
    Stop-Gate "Tutor 2.3.2 tests changed production data, learning memory, progress, retrieval indexes, or configured vault Markdown."
}
...
if ($statusAfter.Count -ne 0) {
    ...
    Stop-Gate "Tutor 2.3.2 tests left repository changes behind."
}
...
Write-Host ""
Write-Host "======================================================================="
Write-Host " ANVAYA TUTOR 2.3.2 DIAGNOSTIC QUESTIONING: PASS"
Write-Host "======================================================================="
Write-Host "Bounded diagnostic questioning, explicit-request bypass, and one-question provider planning are green."
Write-Host "Tutor 2.3.1 session-goal/teaching-plan behavior, Tutor 2.2 memory/personalization,"
